...
# Değiştirildi: yerel kameradan ziyade MJPEG akışını kullan
...
while True:
    ret, frame = cap.read()
    if not ret:
        print("Akıştan frame alınamıyor, yeniden bağlanmaya çalışılıyor...")
        try:
            cap.release()
        except:
            pass
        time.sleep(1)
        cap = cv2.VideoCapture(STREAM_URL)
        # isteğe bağlı: tekrar denemek için döngüye devam et
        continue
    ...
    frame_inpainted = cv2.inpaint(frame, mask_parlak, inpaintRadius=5, flags=cv2.INPAINT_TELEA)
    ...
    for c in contours:
        ...
        # Dairesellik Oranı Hesaplama
        if perimeter > 0:
            dairesellik_orani = (4 * np.pi * area) / (perimeter ** 2)
        else:
            dairesellik_orani = 0
        ...
        if M["m00"] != 0:
            ...
            current_max_area = MAX_LED_ALAN 
            if cY < yarim_yukseklik:
                
                current_max_area = int(MAX_LED_ALAN * MAX_ALAN_FAKTOR_UST)
            
           
            if area > MIN_LED_ALAN and area < current_max_area and dairesellik_orani > DAIRESELLIK_ESIGI:
    ...
    showinWindow(PENCERE_ADI, frame_inpainted)
    showinWindow("2. Maske (Renk Izolasyonu + Filtreleme)", mask)
    showinWindow("3. Inpaint Maskesi", mask_parlak)  
